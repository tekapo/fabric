...
よくある質問とそれに対する答え、解決法、言い訳は :doc:`faq` のページで見つかるかもしれません。
...
助けを求めたりバグを登録する前に、 :doc:`document on troubleshooting <troubleshooting>` を必ず読んでください。
...
Fabricは2つのAPIドキュメントをメンテナンスしています。これはソースコードのdocstringsから
自動生成されるもので、通常はとても詳細なものです。
...
**コア** APIは、Fabricの基礎的な構成要素 (`~fabric.operations.run` や `~fabric.operations.sudo` など)
を形成する関数、クラス、メソッドとしておおまかに定義されています。そしてこの上に他のすべて(以下の
"contrib" セクションやユーザーのfabfile)が作られています。
...
Fabricの **contrib** パッケージにはユーザーI/O、リモートファイルの修正などのタスクように
に一般的で便利なツールが含まれています(たいていはユーザーのfabfileからマージされています)。
coreのAPIが小さくサイズに保たれ比較的長期間にわたって変更されないのに対し、このcontribセクションは
ユーザーケースが解決されて追加されるごとに成長し、(後方互換性は保つようにしてはいますが)進化します。
...
もし :ref:`使い方<usage-docs>` や :ref:`API <api_docs>` ドキュメントを探し回っても
答えが見つからない場合は、以下の様々なリソースが助けになるはずです。ただし、
チケットを投稿したり質問をメーリングリストに投げかける前に、最低でもドキュメントには
ざっと目を通すようにお願いします。
...
Fabricの使用に関して助けを求める一番いい場所は `fab-user メーリングリスト
<http://lists.nongnu.org/mailman/listinfo/fab-user>`_ (いまは ``nongnu.org``
でホストされています)でしょう。Fabricの開発者はすぐに返信するようにしていますし、
このリストには他のFabricユーザーや貢献者の活発なコミュニティも参加しています。
...
Fabricには公式のTwitterアカウント `@pyfabric <http://twitter.com/pyfabric>` があります。
これはアナウンスに使用されたり、たまに関連するニュースや情報 (例えば「Fabricに関するいい投稿記事が
あるよ!」) を流したりします。
...
新しいバグの報告や既存のバグの検索は `Github Issues <https://github.com/fabric/fabric/issues>`
ページで行えます。これには (無料で簡単に登録できる) GitHubのアカウントが必要です。
...
返信できないかもしれませんが、このチャンネルのログを取っている方がいるので可能なときには返信するようにしています。